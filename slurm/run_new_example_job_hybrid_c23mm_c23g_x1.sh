...
if [[ -n "$job_ids" ]]; then
    JOB_OUTPUT=$(sbatch --dependency=afterany:$job_ids slurm/new_example_job_hybrid_c23mm_c23g_x1.sh M "0.0")
else
    JOB_OUTPUT=$(sbatch slurm/new_example_job_hybrid_c23mm_c23g_x1.sh M "0.0")
fi
